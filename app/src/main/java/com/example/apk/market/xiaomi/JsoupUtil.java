package com.example.apk.market.xiaomi;

import com.example.apk.market.manage.local.LocalApp;
...
import static com.example.apk.market.xiaomi.XiaoMiInterface.BASE_URL;
...
//            Log.d("test", appInfo.toString());
    ...
    public static AppInfo getDetailInfo(String result, AppInfo appInfo) {
        ...
        if (document == null) return appInfo;
...
//        Log.d("test", "图标:" + icon);
        ...
        String size = document.select(".container.cf .float-left div[style=float:right;]").get(0).text();//软件的大小
...
//        Log.d("test","评论："+commentNum);
        String updateTime = document.select(".container.cf .float-left div[style=float:right;]").get(2).text();;
//        Log.d("test", "更新时间:" + updateTime);
        String packageName = document.select(".container.cf .float-left div[style=float:right;]").get(3).text();;
//        Log.d("test", "包名：" + packageName);
        String versionCode = document.select(".container.cf .float-left div[style=float:right;]").get(1).text();;
//          Log.d("test","版本号："+versionCode);
        List<String> permissionList = new ArrayList<>();//所需要的权限
        Elements elements2 = document.select("div.details.preventDefault span[style=color: #343434]");
        for (int i = 0; i < elements2.size(); i++) {
            permissionList.add(elements2.get(i).text());
//            Log.d("test", elements2.get(i).text());
        }
        String downloadUrl = document.select("div.app-info-down").select("a").attr("src");
        if (!StringUtil.isBlank(downloadUrl)) downloadUrl = BASE_URL + downloadUrl;
